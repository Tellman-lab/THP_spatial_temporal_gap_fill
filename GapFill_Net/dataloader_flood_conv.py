...
class FloodSTDataset(Dataset):
    ...
    def __init__(self, data_root, split,event,
        ...
        self.dem_file = osp.join(data_root,"DEM",self.event)
        self.slope_file = osp.join(data_root,"SLOPE",self.event)
        self.twi_file = osp.join(data_root,"TWI",self.event)


        self.valid_idx = np.array(
            range(-idx_in[0], self.series_path.__len__()-idx_in[-1]))
    ...
    def __getitem__(self, index):
        ...
        for i in range(index+self.idx_in.min(),index+self.idx_in.max()+1):
            if i == int((index+self.idx_in.min()+index+self.idx_in.max())/2):
            ...
            # S1_array = read_tif_as_numpy(self.PS_files[i])
        ...
        dem = read_tif_as_numpy(self.dem_file)
        slope = read_tif_as_numpy(self.slope_file)
        twi = read_tif_as_numpy(self.twi_file)



        aux_data =np.array([dem,slope,twi])
        aux_data_ = []
        for b in range(aux_data.shape[0]):
            aux_data_.append(cv2.resize(aux_data[b,0,:,:], (128, 128),interpolation = cv2.INTER_LINEAR))

        aux_data_ = np.array(aux_data_)
        ...
        sample = {"image": data, "label": labels,"aux":aux_data_}
...
if __name__ == '__main__':
    ...
    step = 1
    train_set, test_set = \
                load_data(data_root=r'/home/convlstm_predict/FuseNet_Interp_v1/custom_fl_dataset/time_series_dataset_v1/CMO',
                event="0a7e.tif",
                idx_in=[-1,0,1],
                idx_out=0,
                step=step, use_augment=True)
    train_data,label_data,aux_data = train_set[1]['image'],train_set[1]['label'],train_set[1]['aux']
